...
# Define base paths
AEON_HOME="/home/nexus/AeonCore"
CONFIG_PATH="$AEON_HOME/config"
...
if [[ -f "$CONFIG_PATH/aeoncore.conf" ]]; then
    source "$CONFIG_PATH/aeoncore.conf"
    echo "[+] Config loaded." >> "$LOG_PATH/startup_$NOW.log"
else
    echo "[!] Config not found at $CONFIG_PATH/aeoncore.conf" >> "$LOG_PATH/startup_$NOW.log"
    echo "Exiting."
    exit 1
fi
